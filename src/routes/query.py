...
async def query_project(
    ...
    try:
        ...
        final_llm_response = litellm.completion(
            **llm_params,
            messages=[{"role": "user", "content": prompt}],
        )
        token_tracker = LLMUsageTracker()
        token_tracker.track_response(response=final_llm_response, auth_token=auth_token, model=llm_params.get("model", ""))
        llm_response = final_llm_response.choices[0].message.content.strip()
